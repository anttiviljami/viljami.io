---
title: "Testing React With Jest and OpenAPI mocks"
date: "2020-06-12"
description: "Kent C. Dodds recently wrote a very interesting post calling an end to mocking window.fetch when..."
...
{% twitter 1270359083409965057 %}

He was right.

I just recently had to migrate a React project from a fetch based API client implementation to an [axios based one](https://github.com/anttiviljami/openapi-client-axios) where the tests heavily relied on mocking `global.fetch`. It very quickly became apparent why this is not good practice.
...
As a better alternative, Kent suggests using a *Mock Service Worker*. More specifically the [msw module](https://github.com/mswjs/msw) to essentially run a mock backend as a service worker that intercepts all outgoing API requests to handle them.

### Setting up msw

Setting up a mock backend with msw for your React tests turns out to be a fairly easy process. To get a deeper picture you should check out [Kent's original post](https://kentcdodds.com/blog/stop-mocking-fetch), but here's all you really need to do in your test code to mock a REST endpoint:
...
As someone who works a lot with API backends that (hopefully!) provide Swagger/OpenAPI definitions, I had already been mocking my backends in React tests using OpenAPI mocks with [openapi-backend](https://github.com/anttiviljami/openapi-backend). When I learned about `msw`, I was thrilled!
...
To enable request validation during tests, you can simply register the [validationFail handler](https://github.com/anttiviljami/openapi-backend/blob/master/DOCS.md#validationfail-handler) for openapi-backend:
...
Finding out about msw was a major game changer for my React testing. This in combination with the auto-mocking capabilities of [openapi-backend](https://github.com/anttiviljami/openapi-backend) makes mocking APIs in React tests a breeze.


Thank you, Kent, and the team behind [mswjs/msw](https://github.com/mswjs/msw)! 💙
